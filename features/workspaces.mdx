---
title: '⬇️ Partial Clones'
description: 'Oxen allows you to interact with your data without having to download the entire dataset locally.'
---

Say you want to contribute to a dataset with 100GB of images, or only need a small subset of the data to run a model. In these cases, it doesn't make sense to download the entire dataset locally. Instead, you can use partial clones.

Oxen has three main ways of interacting with subsets of your data.

1) **Download Read Only** - Download a read only copy of the subset to your local machine.
...
Under the hood, this command does not download any of the history, version files, or other metadata. It simply downloads the data unpacks it to a local directory.
