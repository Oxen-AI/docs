...
In order to use embeddings, you will need to create a [workspace](/getting-started/workspaces). Workspaces allow you to query and edit versions of the data without immediately committing your changes. Oxen uses [DuckDB](https://duckdb.org/2024/05/03/vector-similarity-search-vss.html) to store your embeddings and data.
...
Now we have our embeddings committed to the repository. We can create a workspace to query the data. A workspace is based off of a branch and links directly to a version of a dataset at a commit. If you want to learn more about workspaces, check out the [workspaces](/getting-started/workspaces) page.
...
If you want to see the underlying HTTP request that is being made, checkout the [API reference](/http-api/data-frames/get-data-frame-slice).
