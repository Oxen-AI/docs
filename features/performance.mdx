...
Another popular choice for storing datasets, Hugging Face hosts several variations of ImageNet on its hub. For the repositories that contain the full 1 million+ files, they appear to all compress the data into small amounts of tarballs or parquet files. For example, [ILSVRC/imagenet-1k](https://huggingface.co/datasets/ILSVRC/imagenet-1k), their most popular ImageNet, stores the images as 5 ~30-GB tarballs, which we tested.        

Steps to reproduce:

```
tar czf imagenet-images.tar.gz images/ # Executed in  114.66 mins
split -n 5 -d imagenet-images.tar.gz imagenet-part.tar.gz # Executed in  minutes
...
As Git-LFS was prohibitively slow uploading ImageNet, we began this testing with DVC
...
Unlike with Hugging Face, this will download the uncompressed files, saving the cost of extracting them from tarballs or parquet. You can also clone the repository to have the whole thing in a local oxen repo.
