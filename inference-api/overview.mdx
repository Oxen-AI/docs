...
The Inference API gives you access to hundreds of AI models through a single, consistent interface. Generate text, images, videos, and audio without managing infrastructure or juggling multiple provider SDKs.

There are two doors. Text models answer inside the request, through chat completions. Everything that produces a file, meaning images, video, and audio, is queued: you submit a job, get an ID back, and collect the result once it is ready.
...
<CardGroup cols={3}>
  ...
  <Card title="Media Generation" icon="layer-group" href="/inference-api/reference/async_queue">
    Queued image, video, and audio generation
  </Card>
...
| `/ai/queue` | POST | Submit an image, video, or audio generation |
...
| `/ai/queue/:generation_id/retry` | POST | Kill a generation and run it again |
| `/ai/generations` | GET | Browse generation history, paginated, with cost per row |
| `/api/events` | GET | Event stream that fires when a generation finishes |
