...
description: "Generate text responses from language models with support for streaming, vision, documents, and tool calling"
---
...
Compatible with the OpenAI chat completions format. Supports streaming, multimodal input (images, video, and documents), tool calling, and structured output.
...
Attach a document to a message with a `file` content part. The OpenAI-compatible
way (works unchanged through the OpenAI SDK) is to pass the file inline as a
base64 [data URL](https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/Data_URLs)
in `file.file_data`, exactly like OpenAI's
...
[workspace](/concepts/workspaces). This is an **Oxen.ai extension**: OpenAI's
Chat Completions `file` object has no URL field (only `file_data` and `file_id`),
so we borrow the `file_url` field name from OpenAI's
[Responses API](https://platform.openai.com/docs/api-reference/responses):

```json
{
  "type": "file",
  "file": {"file_url": "https://hub.oxen.ai/api/repos/ox/playground/file/main/report.pdf"}
}
```

Pass exactly one of `file_data` (a base64 data URL) or `file_url` (a URL). The
URL must be publicly accessible (and unexpired, if presigned).

The document works the same regardless of which model you call — Oxen.ai
translates it to each provider's native format (an Anthropic `document` block,
an OpenAI `file` part, a Gemini `inline_data` part, and so on). Place the
document before the text part for the best results.
...
{
  "error": {
    "message": "Unsupported file type 'image/tiff'. Supported file types: application/pdf."
  }
}
```
