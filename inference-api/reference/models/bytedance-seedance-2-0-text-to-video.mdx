...
description: "Cinematic text-to-video, up to 4k"
---
...
ByteDance Seedance 2.0 (Pro) generates video from a text prompt. Supports 480p, 720p, 1080p, and 4k output, durations from 4-15 seconds, multiple aspect ratios, and optional synchronized audio.
...
|-------|------|---------|-------------|
| `prompt` | `string` | `"A lone ox walks down an empty desert highway at golden hour, dust drifting behind it, slow cinematic camera move, warm low sunlight, shallow depth of field."` | The text prompt used to generate the video. |
...
| `generate_audio` | `boolean` | `false` | Whether to generate synchronized audio for the video, including sound effects, ambient sounds, and lip-synced speech. |
