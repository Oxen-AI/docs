---
title: "LTX 2.3 Retake"
description: "Regenerate a section of a clip"
---

<CardGroup cols={1}>
  <Card title="Try LTX 2.3 Retake in the Workbench" icon="flask" href="https://www.oxen.ai/ai/workbench?model=ltx-2-3-retake">
    Run this model interactively, tune parameters, and compare outputs.
  </Card>
</CardGroup>

**Model ID:** `ltx-2-3-retake`

LTX 2.3 Retake regenerates a section of an existing clip guided by a prompt — change a line of dialogue, an action, or the sound without touching the rest — served directly by Lightricks' LTX API on the Pro tier. Pick where the section starts and how long it runs, and whether to replace its video, its audio, or both.
...
<Tip>
  Use the [Workbench](https://www.oxen.ai/ai/workbench?model=ltx-2-3-retake) as a request builder: configure parameters for this model in the UI, then open the **API** tab to copy the exact cURL or Python call.
</Tip>
...
```bash
curl -H "Authorization: Bearer $OXEN_API_KEY" https://hub.oxen.ai/api/ai/models/ltx-2-3-retake
```
...
|-------|------|---------|-------------|
| `input_video` | `string` | &mdash; | The clip to retake a section of. Up to 4K, at least 3 seconds. Format: uri. |

### Optional parameters

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `prompt` | `string` | &mdash; | What should happen in the retaken section. |
| `retake_start_time` | `number` | `0` | Where the retaken section starts, in seconds. Range: 0 – +∞. |
| `retake_duration` | `number` | `4` | Length of the retaken section in seconds, at least 2. Range: 2 – +∞. |
| `retake_mode` | `string` | `"replace_audio_and_video"` | Replace the video, the audio, or both in the retaken section. One of: replace_audio_and_video, replace_video, replace_audio. |
