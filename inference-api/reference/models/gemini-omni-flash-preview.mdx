...
Gemini Omni Flash is Google's omni-modal model that generates short videos with synchronized audio from a text prompt, input images, or an input video to edit. It brings multimodal creative direction into a single model, producing 3 to 10 second 720p MP4 clips.
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `input_image` | `string` | &mdash; | Optional image to use as the video's starting frame. Reference it in the prompt as @FirstFrame. Format: uri. |
| `input_images` | `array<string>` | &mdash; | Optional reference images (characters, objects, styles to carry into the video). Reference them in the prompt as @Image1, @Image2, etc. |
