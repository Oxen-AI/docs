---
title: "Muse Image 1.0"
description: "Fast image generation and precise editing"
---

<CardGroup cols={1}>
  <Card title="Try Muse Image 1.0 in the Workbench" icon="flask" href="https://www.oxen.ai/ai/workbench?model=muse-image-1-0">
...
<Tip>
  Use the [Workbench](https://www.oxen.ai/ai/workbench?model=muse-image-1-0) as a request builder: configure parameters for this model in the UI, then open the **API** tab to copy the exact cURL or Python call.
</Tip>
...
```bash
curl -H "Authorization: Bearer $OXEN_API_KEY" https://hub.oxen.ai/api/ai/models/muse-image-1-0
```
...
|-------|------|---------|-------------|
| `prompt` | `string` | &mdash; | Text description of the image to generate, or of the edit to apply to the reference images. |

### Optional parameters

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `input_image` | `array<string>` | &mdash; | Optional reference image(s). Pass one to edit it, several to compose them, or leave empty to generate from the prompt alone. |
| `aspect_ratio` | `string` | `"16:9"` | Aspect Ratio One of: auto, 1:1, 16:9, 9:16, 4:3, 3:4, 3:2, 2:3, 21:9, 9:21. |
| `thinking_level` | `string` | `"high"` | How much the model reasons before generating. 'high' improves complex compositions at higher latency; 'low' renders faster. One of: low, high. |
