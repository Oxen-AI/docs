...
|-------|------|---------|-------------|
| `prompt` | `string` | `"Image 1 walks through a beautiful garden, cinematic lighting."` | Text prompt describing the desired video. Use 'Image 1, Image 2, ...' to reference reference_images in order. |

### Optional parameters

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `reference_images` | `array<string>` | &mdash; | Array of up to 9 reference image URLs for character/object/scene appearance. Order maps to 'Image 1', 'Image 2', etc. Formats: JPEG, JPG, PNG. Max 10 MB each. 720p or higher recommended; shorter side must exceed 400 pixels. Reference images should share a similar aspect ratio close to the target video's aspect ratio. |
