---
title: "LTX 2.3 Pro: Retake"
description: "Targeted video segment editing: replace video, audio, or both via prompts."
---

<CardGroup cols={1}>
  <Card title="Try LTX 2.3 Pro: Retake in the Workbench" icon="flask" href="https://www.oxen.ai/ai/workbench?model=ltx-2-3-pro-retake">
    Run this model interactively, tune parameters, and compare outputs.
  </Card>
</CardGroup>

**Model ID:** `ltx-2-3-pro-retake`

Targeted video editing from Lightricks LTX-2.3 Pro. Regenerates specific segments of existing videos (2+ seconds) while preserving motion continuity, lighting, and surrounding footage. Supports replace video only, replace audio only, or replace both with natural language prompts. 1080p output.

## Example request

<Tip>
  Use the [Workbench](https://www.oxen.ai/ai/workbench?model=ltx-2-3-pro-retake) as a request builder: configure parameters for this model in the UI, then open the **API** tab to copy the exact cURL or Python call.
</Tip>
...
```bash
curl -H "Authorization: Bearer $OXEN_API_KEY" https://hub.oxen.ai/api/ai/models/ltx-2-3-pro-retake
```
...
|-------|------|---------|-------------|
| `prompt` | `string` | &mdash; | Text prompt describing the edit to make |
| `input_video` | `string` | &mdash; | Input video for retake editing Format: uri. |

### Optional parameters

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `task` | `string` | `"retake"` | The generation task (fixed for this model) One of: retake. |
| `retake_start_time` | `number` | `0` | Start time in seconds of the section to edit Range: 0 – +∞. |
| `retake_duration` | `number` | `2` | Duration in seconds of the section to edit. Must be at least 2 seconds. Range: 2 – +∞. |
| `retake_mode` | `string` | `"replace_audio_and_video"` | What to replace in the retake section One of: replace_audio, replace_video, replace_audio_and_video. |
