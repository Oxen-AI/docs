...
Generations persist after reaching a terminal state (`succeeded`, `failed`, or `cancelled`), so you can retrieve their status at any time. The link they carry to the output file is short-lived; see [Where results go](#where-results-go).
...
| `target_namespace` | string | your username | Namespace that stores the results and pays for them. An organization name works if you are a member. |
| `target_repo` | string | `playground` | Repository the files are written to. Created on first use. |
| `target_directory` | string | repo root | Directory inside that repository. |
...
A generation writes its output into a repository in your namespace and records the parameters that
produced it alongside the file. That is what makes a run reproducible, and it is why a few weeks of
generations is already a dataset you can version, share, or fine-tune on.

Everything lands in a repository called `playground` unless you say otherwise. An application built
on this API will pile up files there, so give it a repository of its own with `target_repo` and
`target_directory`.

### Result URLs

`result_url` is a signed link to that file. The signature is good for one hour from the moment the
generation completes, and the stored link is never refreshed, so a generation you come back to a day
later hands you a link that expired long ago.

Drop the query string and authenticate with your API key to fetch the same file at any point
afterwards:

```bash
# Good for the first hour.
curl -o output.png "$RESULT_URL"

# Good indefinitely.
curl -o output.png -H "Authorization: Bearer $OXEN_API_KEY" "${RESULT_URL%%\?*}"
```

Download while the link is warm, or keep the bare path and authenticate when you need the bytes.
...
| `url` | yes | no | Signed link to the output file, good for one hour; see [Result URLs](#result-urls). |
