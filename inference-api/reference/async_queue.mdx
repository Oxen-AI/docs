...
description: "Queue image, video, and audio generations and collect the results"
---

Every image, video, and audio model runs through one pair of endpoints. You submit a job to
`POST /api/ai/queue`, get a generation ID back immediately, then poll that ID or listen on an event
stream until it reaches a terminal state. Text models answer inside the request, through
[chat completions](/inference-api/reference/chat_completions).

Media generation is queued because the work takes anywhere from five seconds to fifteen minutes,
which is longer than an HTTP connection should be asked to survive. Queueing also lets many
generations run at once, up to 4 per request with no limit on the total queued, without holding a
...
The parameters most models share, by modality:
...
|---|---|
| `prompt` | What to draw. |
| `aspect_ratio` | Shape of the output. Which values a model accepts varies, and an unsupported one can come back as the model's own default, so check the dimensions of what you get. |
| `num_inference_steps` | Denoising steps. Higher values take longer. |
| `seed` | Fixes the random draw, so the same request reproduces. |
...
| `input_image` | The image to work from. Some models take one URL and some take an array; the model's schema says which. |

The model downloads `input_image` itself, so the URL has to be reachable without credentials. Data
URIs work and bloat the request, so prefer a hosted file.
...
| Parameter | Description |
|---|---|
| `prompt` | What happens in the shot. Use this or `multi_prompt`. |
| `multi_prompt` | A prompt and a duration per shot, for models that cut between shots inside one generation. |
| `duration` | Length in seconds, when using `prompt`. |
| `aspect_ratio` | Shape of the output. |
| `input_image` | Starting frame or reference images, depending on the model. |
| `input_video` | The clip a video-to-video model works from. |
| `generate_audio` | Ask for an audio track, where the model can make one. |
...
| Parameter | Description |
|---|---|
| `prompt` | The words to speak, or a description of the sound. |
| `audio_urls` | Reference clips, for cloning a voice. |
| `output_format` | Container for the result, such as `mp3`. |
...
Model-specific parameter validation happens **when the generation runs**. A misspelled parameter, a duration the model refuses, an unreachable `input_image`, or no `prompt` at all will each return `200` with a generation ID and then surface seconds later as `status: "failed"` with an `error_message`. A successful enqueue tells you the request was accepted, so read `error_message` on every failure.
