...
|---|---|---|---|
| `prompt` | string | -- | Text prompt. Per-model requirements vary (some models use `multi_prompt`; upscalers and video-to-video models use `input_image` or `input_video` instead). Not enforced at enqueue time; consult the model's `request_schema`. |
...
| `aspect_ratio` | string | -- | Aspect ratio (e.g. `"16:9"`, `"1:1"`). |
