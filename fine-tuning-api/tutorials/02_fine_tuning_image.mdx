...
With the deployment live, you submit edits to the media queue using the deployed model
identifier, exactly as you would for a base model.
...
curl -X POST \
  "${OXEN_BASE_URL:-https://hub.oxen.ai}/api/ai/queue" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer ${OXEN_API_KEY}" \
  -d "{
...
Each call returns a generation ID. Poll `GET /api/ai/queue/<generation_id>` until its `status`
is `succeeded`, then read `result_url`:

```bash
curl -s -H "Authorization: Bearer ${OXEN_API_KEY}" \
  "${OXEN_BASE_URL:-https://hub.oxen.ai}/api/ai/queue/${GENERATION_ID}" | jq -r '.result_url'
```
...
For the queue itself, including the event stream and how long result URLs stay valid, see the
