...
You can think of the keys as the fields that are hashed to create a unique id and the targets as the fields that are compared to tell if a row is added, removed, or modified. To see this in action, let's add one row, remove one row, and modify 3 existing ones to demonstrate how this works.
...
A diff that only specifies a key will show the correct number of changes, but it may have many columns that are not relevant to the changes you are interested in. This is where the `-t` flag comes in handy.
...
Oxen will save the results of the diff to a new file called `diff.csv`. You can then load it into a jupyter notebook or any other tool to do more analysis on the results.

## Real World Example

To drive this home, imagine you have taken this dataset and run it through an LLM with a prompt to get the responses. You have saved the results in a new file called `model_results.csv`.
