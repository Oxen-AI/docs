...
Each schema has an `Optional<String>` of metadata that can be added to the root of the schema. This is useful for adding information about the schema itself. For example, you can add a description of the schema or a json blob that gives context to a data renderer.
...
```bash
oxen schemas add annotations/train.csv -c 'file' -t 'path' -m '{"root": "images/"}'
```
...
The `-c` flag stands for `column`, the `-t` flag stands for `type`, and the `-m` flag stands for `metadata`. You can pass in an arbitrary string for metadata, but it is nice to pass in some structured data such as json.

Notice, it does not change the original detected type, but updates the `dtype_override` for the datatypes on the schema. This is because the overrides get applied on read, not on write.
