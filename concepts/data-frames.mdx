...
With Oxen you can interact with data frames that are not downloaded to your local machine. This can be useful for data collection, labeling workflows, or quickly inspecting data without having to download it.

See the [Oxen Workspaces](/concepts/workspace) documentation for more information on how to edit data directly the oxen-server.
...
# Create the workspace with the latest commit off main
workspace = Workspace(repo, "main")

# Connect to the data frame
data_frame = WorkspaceDataFrame(workspace, "train.csv")

# Add a row
row_id = data_frame.insert_row({"file": "images/000000128154.jpg", "label": "dog"})
...
# Update a row
row = data_frame.update_row(row_id, {"label": "cat"})
print(row)
