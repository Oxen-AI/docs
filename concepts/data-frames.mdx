...
To follow along with the examples below feel free to grab the example data from our public [LLM-Zero-Hero](https://hub.oxen.ai/datasets/LLM-Zero-Hero) repository.

```bash
mkdir my-dataset
cd my-dataset
oxen download datasets/LLM-Zero-Hero finetune/SMSSpamCollection.tsv
```

Then push the data to a repository of your own.

```bash
oxen init
oxen create-remote --name my-username/my-dataset
oxen config --set-remote origin https://hub.oxen.ai/my-username/my-dataset
oxen add SMSSpamCollection.tsv
oxen push
```
...
See the [Workspaces](/concepts/workspaces) documentation for more information on how editing data works under the hood.

```python
from oxen import DataFrame

# Connect to the data frame
data_frame = DataFrame("my-username/my-dataset", "SMSSpamCollection.tsv")

# Add a row
row_id = data_frame.insert_row({"category": "spam", "message": "CLICK HERE TO WIN INSTANTLY."})
...
# Update a row
row = data_frame.update_row(row_id, {"category": "new_category"})
print(row)
...
# Commit the changes
data_frame.commit("Update label")
```
