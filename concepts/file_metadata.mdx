...
the change against a commit, so metadata can be set without committing first —
...
This is the HTTP equivalent of `oxen schemas add <path> -m '{...}'`.
...
This is the HTTP equivalent of `oxen schemas add <path> -c 'file' -m '{...}'`, and
