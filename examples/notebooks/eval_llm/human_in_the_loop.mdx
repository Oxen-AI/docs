---
title: 🕵️‍♂️ LLM Evaluation
description: 'How to build a human in the loop evaluation workflow.'
---

One of the most reliable ways to evaluate an LLM and make sure it is aligned with your company's needs is to have a human in the loop reviewing each input and output pair. This tutorial will show you how to build a simple labeling tool that allows a human to review the output of an LLM, and give a thumbs up or down. You could extend this to have a reason for their answer, or even a score.

Once you have a dataset of labeled examples, you can use it to [train a new model](/examples/notebooks/train_llm), or compare the performance of different models.
...
Follow along with the [example notebook](/examples/notebooks/eval_llm/human_in_the_loop) by running it in your own Oxen.ai account.
