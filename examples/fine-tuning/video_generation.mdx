...
That returns a generation ID. Poll `GET /api/ai/queue/<generation_id>` until its `status` is
`succeeded` and read `result_url`. See the
[media generation reference](/inference-api/reference/async_queue) for the full flow.
