...
Attach a document with a `file` content part. Oxen.ai translates the document to
each provider's native format, so it works the same regardless of which model
you call. Place the document before the text part for the best results.
