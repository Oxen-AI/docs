...
Only `application/pdf` is supported today; other file types return a `400`.
Referencing files by OpenAI `file_id` is not supported — inline the file with
`file_data` or pass `file_url`.
