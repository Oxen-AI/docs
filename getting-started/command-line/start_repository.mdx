...
Setting a remote reads the repository from the server and records its identifier alongside the name and URL, so the remote has to be reachable and the repository has to exist. Create the remote repository first if it does not exist yet.

Pointing a remote that is already set at a different repository is refused, since the identifier recorded for it no longer matches what the new URL serves. Delete the remote with `oxen config --delete-remote origin` and set it again to attach it to a different repository.
