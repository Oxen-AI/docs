---
title: 🐂 What is Oxen?
description: 'Oxen.ai is a developer platform for fine-tuning and customizing models on your own proprietary datasets.'
---

<img alt="Oxen.ai Moon Ox Hero" classname="block" src="/images/MoonOx.png" />

Oxen.ai gives you the tools to select the right [model and prompt](/getting-started/inference). Bring your own [datasets](/features/datasets) to [evaluate](/features/evaluation), [fine-tune](/features/fine-tuning), and own your model end-to-end. Everything including datasets, model weights, and [notebooks](/features/notebooks) is versioned for easy iteration, comparison, and collaboration.

Great models are built through constant iteration and clarity of goals - whether that's higher quality, lower cost, faster response, or domain-specific accuracy. Oxen.ai helps you get there, faster.
 
## ✅ Features

Oxen.ai's platform allows you to quickly graduate from prompt engineering to custom model deployment when needed. The platform's features include:
...
With easy-to-use developer tools, you have access to the raw model weights, datasets, and code through the [command line interface](/getting-started/cli), [python library](/python-api/index) or [HTTP API](/http-api/index). This makes it seamless to integrate Oxen into your existing workflows and train on your own private data.
...
Closed source models not working for your use case? Fine-tune your own model, optimizing it for speed, privacy or cost, and deploy it to the same interface in minutes.
...
TODO: Form for early access to multi-node training
