...
description: 'Oxen.ai is built on top of a scalable data version control system that allows you to version, branch, and share datasets, model weights, and experiments with your team.'
---

Oxen's [open source data version control system](https://github.com/Oxen-AI/oxen-release) shines at workflows and data sizes that git or git-lfs fall short. The interface is inspired by git, so that it is easy to learn, but has a few core differences. Oxen is built from the ground up to handle large datasets with many files or large csvs, parquet files, or other large binary blobs like model weights.

## Versioning 101

While Oxen is inspired by git, the first thing you need to know about Oxen.ai is that it has both remote and local workflows. Remote workflows allow you to add files directly to the remote without pulling any data locally. Say we wanted to add a file to a dataset like ImageNet with [1 Million Files](/features/performance), you do not want to wait to clone all the files locally just to add yours.
...
Remote repository workflows are best suited for large datasets where it would be expensive to have a fully copy of the data on your local machine. This is the biggest difference between Oxen and git. Oxen can work in both a centralized and decentralized version control paradigm. 

TODO: Image of a centralized and decentralized workflow

While the decentralized nature of git makes it easy to maintain full copies of the history across many machines, this is not practical for large datasets. Oxen was designed from the ground up to be able to seamlessly switch between local and remote workflows.
...
TODO: We need a better explanation of workspaces.
