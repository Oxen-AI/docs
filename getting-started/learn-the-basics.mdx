...
Oxen.ai makes versioning your datasets as easy as versioning your code. You can install through homebrew or pip or from our [releases page](https://github.com/Oxen-AI/Oxen/releases).
...
Clone your first Oxen.ai repository from the [OxenHub](https://oxen.ai/explore).
...
Initialize your first Oxen.ai repository, and commit the first version of your data.
...
After initialize your local Oxen.ai repository or cloning a repository from the [OxenHub](https://oxen.ai/explore), you can create a remote repository and push your data.

<CodeGroup>

```bash CLI
# Create a remote Oxen.ai repository 
oxen create-remote --name USERNAME/REPO_NAME
# Configure an existing local repository to the remote repository
oxen config --set-remote origin https://hub.oxen.ai/USERNAME/REPO_NAME
# Push your data to the main branch of theremote repository
...
Confidently overwrite the file, move the file, delete the file, it doesn't matter. Oxen.ai will always have a copy of the data at the time of the previous commit.
...
The diff command line tool is more powerful than it looks on the surface. Oxen.ai has the ability to diff files of many formats, and the ability to specify keys are targets in tabular diffs to make it easier to see what changed.
...
Clone your data faster than ever before. Oxen.ai has been optimized to the core to make pulling large datasets as fast as possible.
...
Only pull the changes you need. Oxen.ai will only pull the files that have changed since the last time you pulled.
...
With Oxen.ai you do not need to download the entire dataset to your local machine. You can download only the subset of files or directories you need.
